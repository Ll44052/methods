...
for i in range(n):
    ...
    print(f"Узел {i+1} ({capacity[i]}):", end=" ")
    print(*assign[i], end=" ")
    print(f"- загрузка: {used[i]}/{capacity[i]} = {percent:.1f}%")

# среднее
avg = sum(loads) / n
print(f"\nСредняя загрузка: {avg:.1f}%")
...
print("Отклонения:", end=" ")
for d in devs:
    print(f"{d:.1f}", end=" ")

# среднее отклонение
avg_dev = sum(devs) / n
print(f"\nСреднее отклонение: {avg_dev:.2f}%")
